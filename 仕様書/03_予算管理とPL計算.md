# Rinori 販売管理システム 詳細仕様書 V1.62
## 第3部：予算管理とPL計算

**バージョン**: 1.62  
**作成日**: 2026-04-16  
**基準文書**: Rinori 販売管理システム 要件定義書_V1.62
...
| **売上不足額** | **shortage_amount** | **予算売上 - 実績売上（達成済みの場合は「達成済み」表示）** |
| **必要販売個数** | **required_quantity** | **売上不足額 ÷ 商品単価（予算売上 ÷ 予算数量）、切り上げ（達成済みの場合は「達成済み」表示）** |
...
- **売上不足額**: `予算売上 - 実績売上`
  ...
  - 実績売上 < 予算売上 の場合: 不足額を表示
- **必要販売個数**: `CEIL(売上不足額 ÷ 商品単価)`
  - 実績売上 ≥ 予算売上 の場合: 「達成済み」と表示
  - 実績売上 < 予算売上 の場合: 必要個数を整数で表示（切り上げ）
...
**UI仕様【V1.62追加】**:
- **ヘッダー固定**: テーブルヘッダーを`position: sticky; top: 0`で固定
  - 下にスクロールしても列名が常に表示される
